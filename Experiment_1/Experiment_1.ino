...
void isrTimer(void){
  ...
  if((buttonState!=lastButtonState)&&(buttonState==LOW)){              // Judge of the button is pressed
  ...
  if(((counter==5)&&(mode==HIGH))||((counter==100)&&(mode==LOW))){     // Judge if the state of the LED requires a change
    counter=0;                                                         // Reset the counter
